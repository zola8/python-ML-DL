...
def check_empty_hand_landmark(file_name, detection_result):
    if len(detection_result.hand_landmarks) == 0:
        print(f"Filename: {file_name} - No hand landmark detected")
...
if __name__ == '__main__':
    ...
    for dir_ in os.listdir(DATA_DIR):
        for img_filename in os.listdir(os.path.join(DATA_DIR, dir_)):
            fname = f"{os.path.join(DATA_DIR, dir_, img_filename)}"
            # print("\nImage: ", fname)
            ...
            check_empty_hand_landmark(fname, detection_result)

            # STEP 5: visualize
            # annotated_image = draw_landmarks_on_image(image.numpy_view(), detection_result)
            # cv2.imshow(fname, cv2.cvtColor(annotated_image, cv2.COLOR_RGB2BGR))
